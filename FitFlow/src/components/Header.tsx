          ...
          <nav className="hidden lg:flex items-center space-x-8">
            {navItems.map((item) => (
              ...
              >
                {item.label}
                {activeSection === item.id && (
                  <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-emerald-500 to-blue-500 rounded-full"></div>
                )}
              </button>
            ))}
            <button className="bg-gradient-to-r from-emerald-500 to-blue-500 text-white px-6 py-2 rounded-full font-medium hover:shadow-lg transform hover:scale-105 transition-all duration-300">
              Join Now
            </button>
          </nav>
          ...
          <div className="lg:hidden absolute top-full left-0 right-0 bg-white shadow-lg border-t">
            <nav className="py-4">
              {navItems.map((item) => (
                <button
                  ...
                  className={`block w-full text-left px-6 py-3 font-medium transition-colors ${
                    activeSection === item.id ? 'text-emerald-600 bg-emerald-50' : 'text-gray-700 hover:text-emerald-600 hover:bg-gray-50'
                  }`}
                >
                  {item.label}
                </button>
              ...
              <div className="px-6 pt-3">
                <button className="w-full bg-gradient-to-r from-emerald-500 to-blue-500 text-white px-6 py-3 rounded-lg font-medium">
                  Join Now
                </button>
              </div>
            </nav>
